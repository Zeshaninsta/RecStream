...
const MyPosts = () => {
  ...
  if (loading) {
    return (
      <div className="flex justify-center items-start mt-10 gap-2 text-2xl text-gray-900 min-h-screen">
        <div className="flex justify-center items-center gap-2 ">
          Loading
          <FaArrowsRotate className="animate-spin text-gray-900" />
        </div>
      </div>
    );
  }
  ...
  return (
      <div className="min-h-screen w-full lg:w-[90%] mx-auto flex flex-col items-center m-auto px-2 py-4 ">
        <h2 className="text-2xl font-semibold mb-4 text-gray-900">Your Posts</h2>
        {userName && (
        ...
        {posts.length === 0 ? (
          <p className="text-gray-900 text-center text-md font-rubik">
            No posts found.
          </p>
        ) : (
            ...
            {posts.map((post) => (
              <Link className="w-full" to={`/posts/${post.id}`} key={post.id}>
                <div className="border border-slate-700 shadow-lg cursor-pointer px-2 py-4 w-full mt-2">
                  ...
                  <div
                    className="text-xl w-full  font-rubik font-bold text-gray-900"
